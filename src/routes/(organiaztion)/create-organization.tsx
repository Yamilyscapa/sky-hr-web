...
export const Route = createFileRoute('/(organiaztion)/create-organization')({
...
function RouteComponent() {
  ...
  async function handleCreateOrganization(name: string, slug: string, logo_url: string) {
    try {
      ...
      if (result.data) {
        ...
        if (listResult.data && listResult.data.length > 0) {
          const existingOrg = listResult.data.find(org => org.slug === slug) || listResult.data[0];
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const slug: string = (form.querySelector("#slug") as HTMLInputElement).value;
    const logo_url: string = (form.querySelector("#logo_url") as HTMLInputElement).value;
  ...
  return (
    <div className="container mx-auto max-w-2xl py-8">
      <form className={cn("flex flex-col gap-6")} onSubmit={(event) => handleSubmit(event)}>
        <FieldGroup>
          ...
          <Field>
            ...
            <Input id="name" type="text" placeholder="Mi Empresa S.A." required />
            <FieldDescription>
          ...
          <Field>
            ...
            <Input id="slug" type="text" placeholder="Mi Empresa S.A." required />
            <FieldDescription>
              Como te referirás a la organización en la plataforma (ejemplo: "mi-empresa-s-a").
            </FieldDescription>
          </Field>
          <Field>
            <FieldLabel htmlFor="logo_url">Logo</FieldLabel>
            <Input id="logo_url" type="text" placeholder="URL del logo" required />
            <FieldDescription>
